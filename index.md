# Guia de Mandraque Arqueiro
Contribuintes:

-Nothiel (@nothielf)
-PerSoviNonGrata (@sovimesmo)

Saudações a todos serumanos! Sou o Nothiel [ aka `Nothielf` no discord]. Atualmente jogo de Mandraque desde que lançou a classe. Como tem mta duvida sobre vou centralizar minhas experiencias aqui :) 
Lembrando que esse guia vai ser open source ( link do github [aqui](https://github.com/nothiel/GuiaAbyssChaser)! ), aceitamos pull requests, vamo q vamo.

## Atributos
Com a chegada de `Disparo Frenético`, os atributos mudaram um pouco do padrão de `Disparo Triplo`. Não vou ensinar o que cada atributo faz, pra mais informações usem o [bROWiki](https://browiki.org/wiki/Atributos)!

Abaixo vou deixar como atualmente está a minha build e o pq de cada atributo :)

`FOR 30` Carregar flechas. Cada 5 pontos também garantem +1 de ataque físico base (tem pouca for pq uso mister peso, podem aumentar caso queiram)  
`AGI 94` ASPD precisa fechar 190 para o spam máximo do Disparo Frenético. Mais do que isso pode ajudar a reduzir outras animações, como a de Recuar. Provavelmente vou diminuir isso pq to pegando 192  
`VIT 100` HP e Resistencia a Stun, dá pra aumentar caso queira  
`INT 50` SP. Recomendo deixar mais alta pra n tomar silence, o rim do meu boneco ja n aguenta mais processar pot verde  
`DEX 130` DANO. é isto. Dano.  
`LUK 125` Principalmente pela taxa crítica e para aproveitar os bônus de Bota Temporal SOR. Coloquei 125 por conta da Galensis ~~e nem usei~~, mas dá pra diminuir isso e usar os pontos em int, por exemplo.   

## Talentos
Não sabe oq é talento? [bROWiki](https://browiki.org/wiki/Talentos)!

Aqui Mudará um pouco mais pra frente quando vier os proximos rebalances. Atualmente o peso de CON na formula do `Disparo Frenético` tá baixo, então é mais vantajoso usar POD (por conta do P.ATQ). Atualmente o meu tá:

`POD 100` dano, dale  
`CON 97` mais dano, dale

O resto tá zero!
Testei CON vs CRV por conta do T.CRIT, CON ainda é superior pq entra na formula

## Habilidades
### 1: Esconderijo 10, Furto 10, o resto fodase
### 2: Rajada de flechas no 7, pichar no 1 e preservar, o resto fodase
### 3: Disparo Triplo e Cópia Explosiva no 10, Mimetismo, Escapar e Ofensiva Fatal no 5, o resto fodase
### 4: Disparo Frenético no 10, Sangue Frio no 10, Disparo em Cadeia no 10, o resto fodase

## Equipamentos
Aqui começa a parte complicada. Temos que fechar no mínimo 125 de crit pra não errar o CRIT em monstros com escudo crítico (não sabe o que é escudo crítico? bROWiki, padawan) e 60% de redução de pós-conjuração pra reduzir 0.3s e possibilitar o spam máximo da habilidade.
Nos proximos rebalances isso vai cair pra só 30% pq a recarga vai passar a ser 0.35s ao inves de 0.2s

Abaixo vou colocar opções de builds
### TOPO
`Elmo da Fé II [1] <IT3> <PC3>`: CRIT, CRIT dmg, dano a distância, serve café, comba com arma.  
`Memorável Artifício das Sombras [1] <IT4+>`: CRIT e CRIT Dmg%. Bom, mas perde em dano e crit rate pro Elmo da Fé II.  
`Diadema Temporal Renegado [1] <IT4+>`: Se você for MUITO RICO, é um pouco melhor que o hat de bio5, mas bem mais caro para encantar. Pior que o Elmo da fé, burrice, não vá nesse.
### MEIO
`Óculos Retrofuturistas`: -10% de pós, um terço do efeito de uma carta Kiel.  
`Asas Vitoriosas [1] <Pós>`: Com menos pós overall, mas com Bônus Aleatórios de pós dá pra fechar e ainda tem slot.  
`Selo de Loki [1]`: Consegue até -10% de pós, mas é muito dependente de RNG.  
`Diadema Radiante/Profano [1]`: Situacional contra anjo/demonio/dragão combando com os acessórios da torre de thanatos.
### BAIXO
`Familiar de Combate`: Pós e dano, a diversificação de multiplicadores de dano é muito bem vinda.  
`Competidor Bebê Selvagem`: Pós e dano a distancia. Eu uso esse. Bão bonito e barato.  
`Cachecol Físico de Schmidt`: Usa isso se n fecha CRIT ainda. mas n recomendo mt não.  
`Selo de Ouro`: Dano, comba com Selo de Loki e consegue pegar alguns bônus aleatórios.
### Armadura
`Traje do Lobo Cinzento [1]`: Com Artilheiro e Triplo ele fica bem honesto. melhor que Automatron se bem encantado, mas dificil de vir enchant bom.  
`Malha Corredora Ilusional [1]`: Tem que tá no +11 pra valer a pena. faz combo com a bota, boa demais.  
`Colete Ilusion [1] <U-Pós 2xS-ATQ>`: Bão bonito e acho que a mais barata (Exceto o u-pós, ta o olho da cara).  
`Traje de Freeoni [1]`: Ligeiramente superior ao Ilusion e inferior ao Automatron sem grau. Entre as melhores caso grau A.  
`Colete Automatron [1] <P-Pós, 2xH-Triplo ou 2x M-ATQ>`: Não tive disposição pra por H-Triplo, uso a mesma armor em outros bonecos. bom allrounder e dá 16% de pós, mas é CARO.  
`Exoesqueleto Crítico [1]`: Caro y bom, acho que bem refinado é oq + vai dar dano dessa lista.
### Capa
`Manto Temporal AGI [1]`: Melhor opção de dano entre os Mantos Temporais (ligeiramente supeior ao DES).  
`Manto Temporal DES [1]`: Usável se tu ja fechou seu CRIT Rate.  
`Manto Temporal FOR [1]`: Testei vs o DES, ambos no +12, DES foi melhor, mas se tem esse usa esse.  
`Manto Temporal LUK [1]`: Usa esse se ta dificil fechar CRIT.  
`Jetpack Crítica [1]`: Usa esse se teu cartão virou e vc quer se separar da tua esposa (comba com Exo).  
`Casaco Pirata [1]`: Atualmente BiS pra renegado. Grade C +12 fica MT forte (17.8% mais forte q Temporal DES +12 na minha build).
### Bota
`Bota Temporal LUK [0/1]`: Usa essa. pronto. tenta pegar MA4/DES+200. A ativação constante do encantamento Instinto (DEX+200) pode ser muito custosa em pontos de SP.  
`Sapato Corredor Ilusional [1]` Apenas caso esteja usando o combo com a Malha Corredora Ilusional.
### Arma
`Balista Primordial [2]`: Ligeiramente inferior em dano e resistência em comparação ao Arco Primordial, mas com o bonus de dar -10% de pós. Eu uso essa.  
`Arco Primordial [2]`: Dano bacana, resistencia útil, mas n tem pós, imo pior que Balista Primordial  
`Besta Adulter Fides [2]`: Pior que Arco/Balista primordial mesmo bem refinado, mas se dropar é de graça.  
`Besta Vivatus Fides [2]`: Se usado com o Elmo da Fé 2 fica MUITO mais forte que a balista. tb dá pós conj, maravilhoso (deus pfv deixa eu dropar 01 unidade dele)  
### Acessórios
`Turbinas Ilusión [1]`: Ótimo custo beneficio, encanta com MA4/F4/AA4. Caso sua ASPD já esteja mais que suficiente, o módulo U-Alma pode ser muito bom p ajudar com o consumo de SP   
`Turbinas Automatron [1]`: Mais caro, mas mais forte, uso essa hoje em dia. Encanta com M-Atraso, M-Fatal e M-Tiro  
`Brasão de Schmidt AGI [1]`: Caso combe com o Cachecol, principalmente pela taxa mesmo  
`Combo Radiante/Profano [1]`: Anéis de thanatos encantados, Situacionais: contra anjo/demonio/dragão é bacana pra aumentar dano  

## Cartas
### Topo
`Dramoh Abismal`: Dano Crítico, comba com Carta da Bota (e Meio se tu tiver slot, jogue uma dramoh rei)  
`Bungingis`: Sei lá como fala o nome desse satanás, 5% de hp (e Meio se tu tiver slot, jogue uma deleter voador)  
`Celacanto Violento`: Quer me adotar? BiS.  
`Carta Kiel-D-01`: Quando você é muito rico e acha Bis um doce barato
### Meio
`Dramoh Rei`: Comba com a Dramoh Abismal. se n tiver dramoh abismal usa qlqr outra coisa  
`Deleter Voador`: Comba com a `Buginsalgo`. Dá 5%de hp ela sozinha. acho boa
### Baixo
Sai daqui bRO player safado - (sdds de meu cachecol com Kiel t-t)
### Armadura
`Carta Byorgue`: Um pouco inferior a carta Amdarais, 10% de dano físico e mágico  
`Carta Observação`: Combo com carta `Empatia`. Atualmente é a que uso  
`Carta Raposa Raivosa`: Ajuda se ainda te falta CRIT.  
`Carta Amdarais`: 15% de dano fisico e mágico  
`Carta Amdarais Sombrio`: 20% de dano físico e mágico... me adota?
### Capa
`Carta Ninja Team`: Que saudades do dano a mais do evento :/ anyway, boa carta overall, 200 de atq e uma def honesta (no nível 200).  
`Carta Empatia`: Junto com a Observação dá um dano a distancia ABSURDO, em contrapartida, reduz seu SP máximo. mt forte (uso essa atualmente).  
`Carta Quimera Galensis`: se tá com dificuldade de fechar CRIT essa carta pode te ajudar. Honestíssima.
### Bota
`Carta Tritão Abismal`: Comba com a Dramoh Abismal. ATQ e bastante dano critico, é a que uso atualmente.  
`Carta Zumbi Atirado`: HP e SP, prefiro a Tritão Abismal.  
`Carta Wezen`: Dependendo da quantidade de multiplicadores de dano crítico e dano físico a distância, fica semelhante ao Tritão.

### Acessórios
`Carta Mímico do Vazio`: 15% de dano crítico. A que uso atualmente.  
`Carta Aquecedor Ominoso`: Boa pra quem não fechou CRIT ainda.  
`Carta Dolorian`: Colocaria num radiante pra dar + dano em demônio, mas é situacional.

## Visuais
Uma das partes caras da build  
 
Opção 1- Mortal  
TOPO/MEIO/BAIXO/CAPA - Mortal 1,2,3,4.  
Opção 2- Sicário II.  
TOPO/BAIXO - Algoz II, CAPA Sicário II, MEIO pode ser Alcance ou Mortal.
Outras Opções- Alcance, Pós e C.SP.

## Sombrios
Parte mais cara da build imo.
### Armadura
`Armadura Sombria Penetrante` Usa se tu usar greva pen. Uso essa com CRIT+5.  
`Armadura Sombria do Mastodonte` Usa essa se usar greva mastodonte.  
`Armadura Sombria Autoportante` Entrou recentemente, mt boa e mais barata de fazer que mastodonte.
### Greva
`Greva Sombria Penetrante` Usa se tu usa armor pen. Uso essa com CRIT+5.  
`Greva Sombria do Mastodonte` Usa essa se usar armor mastodonte.  
`Greva Sombria Autoportante` Usa essa se usar armadura.
### Acessórios
Aqui será a mesma coisa: Penetrante, mastodonte ou autoportante. O set ideal é `Penetrante + Mastodonte` ou `Penetrante + Autoportante` . 
### Manopla
`Manopla Sombria Mega Repente` : Entrou recentemente, dá pós e trait boa. uso essa atualmente.  
`Manopla Sombria do Infinito`: Bom pra tirar penalidade de arma.  
`Manopla Sombria POD`: Dá um dano bacana por dar P.ATQ e trait.  
`Manopla Sombria Durável`: Opção honesta pra dar mais dano mas defasada hoje em dia.
### Escudo
`Escudo Sombrio Mega Repente`: Entrou recentemente, dá CRIT e T.CRIT. uso esse atualmente.  
`Escudo Sombrio da Recarga`: -10% de pós e ASPD. maravilhoso, era meu anterior.

## Gameplay
Use `Esfaquear (Deft Stab)`/`Disparo Frenético (Disparo Frenético)` Pra recuperar AP. com 150 de AP usa o `Sangue Frio (Abyss Slayer)`.
em MvP oq eu faço é:   
1- Dou cometa  
2- ATQ Surpresa  
3- Swito pra um acessório com `Carta Esfera Marinha` e uso impacto explosivo  
4- Swito de volta meu acessorio com mimico do vazio  
5-  Rebento o mvp no Disparo Frenético até ele pedir penico

## Conclusão

É isto.
